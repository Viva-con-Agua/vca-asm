		...
		switch( $field['type'] ) {
			...
			case 'avatar':
				...
				if ( empty( $user->simple_local_avatar ) ) {
					echo '<p class="description">' . __( 'No local avatar is set. Falling back to a gravatar, if exists.', 'vca-asm' ) . '</p>';
				} else {
					echo '<span class="box-test"></span><input type="checkbox" name="simple-local-avatar-erase" id="simple-local-avatar-erase" value="1" /><label for="simple-local-avatar-erase"><span class="box"></span>' . __( 'Delete avatar', 'vca-asm' ) . '</label><p class="description">' . __( 'Replace the local avatar by uploading a new avatar, or erase the local avatar (falling back to a gravatar) by checking the delete option.', 'vca-asm' ) . '</p>';
				}
