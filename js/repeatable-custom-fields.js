...
jQuery('.slots-cf-add').click(function() {
	field = jQuery(this).closest('td').find('.slots-cf li:last').clone(false);
	fieldLocation = jQuery(this).closest('td').find('.slots-cf li:last');
	jQuery('input', field).val('').attr('name', function(index, name) {
		return name.replace(/(\d+)/, function(fullMatch, n) {
			return Number(n) + 1;
		});
	...
	slotsSlider();
	slotsRemove();
	var count = jQuery('.slots-cf-remove').length;
	if( count != 1 ) {
		jQuery('.slots-cf-remove').css('display','inline');
	}
	return false;
});

function slotsRemove() {
	jQuery('.slots-cf-remove').click(function() {
		jQuery(this).parent().remove();
		var count = jQuery('.slots-cf-remove').length;
		if( count == 1 ) {
